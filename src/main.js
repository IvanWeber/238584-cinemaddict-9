...
for (let i = 1; i <= NUMBER_OF_FILMS_IN_MAIN_LIST; i++) {
  render(siteFilmsListElement, filmObj.getElementFilmCard(), Position.BEFOREEND);
}

for (let i = 1; i <= NUMBER_OF_FILMS_IN_EXTRA_LIST; i++) {
  render(siteFilmsExtraListElements[0].querySelector(`.films-list__container`), filmObj.getElementFilmCard(), Position.BEFOREEND);
}

for (let i = 1; i <= NUMBER_OF_FILMS_IN_EXTRA_LIST; i++) {
  render(siteFilmsExtraListElements[1].querySelector(`.films-list__container`), filmObj.getElementFilmCard(), Position.BEFOREEND);
}

// Рендер скрытого попапа с детальной информацией о фильме
render(siteBodyElement, filmObj.getElementFilmDetails(), Position.BEFOREEND);
